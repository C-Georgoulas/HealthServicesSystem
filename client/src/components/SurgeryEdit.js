...
export default function PatientDetailsEdit({match}) {
    ...
    useEffect(() => {
        fetch(`/api/patients/${data.state.passedPatientId}/surgeries/${match.params.id}/edit`)
...
const onEditedSurgerySubmit = (e) => {
    e.preventDefault()
    fetch(`/api/patients/${data.state.passedPatientId}/surgeries/${match.params.id}`, {
      method: 'PUT',
  ...
  const form = useRef();
    ...
    return (
      ...
      <Divider />
        ...
        onChange={e => setEditSurgery({...editSurgery, title: e.target.value})}
        validators={['required']}
        errorMessages={['Please add the title of the surgery operation!']}
        />
        </Typography>
        <Divider />
        <Typography variant="body2" component="p">
        ...
        onChange={e => setEditSurgery({...editSurgery, text: e.target.value})}
        validators={['required']}
